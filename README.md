...
The client can combine all publicly known static server and user identifier components, including the servername, port, and username, along with the user secret password and potentially other factors, and process all of it using a rate-limiting function such as bcrypt, scrypt, or pbkdf2.  The resultant hash can be used to seed a prng, and deterministically create a public/private keypair which is unique to that specific user, on that specific server, using that specific password.  Any variation of the input factors results in a completely unrelated keypair.  If a user attempts to login to a malicious site accidentally, the malicious server will only gain knowledge of a derived public key.  The attacker will not be able to impersonate the user at any other server, or even on the compromised server, because the attacker has not discovered either the user's private key for connecting to the compromised server, or the user's password that could be used to derive the private key on this or other servers.  The worst attack a malicious server can mount is an expensive brute force attack, attempting to overpower the rate-limiting function and eventually guess a password that recreates the exposed public key. 
